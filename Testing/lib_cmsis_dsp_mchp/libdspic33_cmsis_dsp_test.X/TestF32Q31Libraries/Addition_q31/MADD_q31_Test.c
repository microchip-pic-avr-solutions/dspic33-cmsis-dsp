...
void MADD_Q31_Test() {
    ...
    ENABLE_PMU;
    mchp_mat_init_q31(&matA, numRows, numCols, MADD_src1_q31);
    mchp_mat_init_q31(&matB, numRows, numCols, MADD_src2_q31);
    mchp_mat_init_q31(&matR, numRows, numCols, MADD_result_q31);
    mchp_status status = mchp_mat_add_q31(&matA, &matB, &matR);
    DISABLE_PMU;
    if (status == MCHP_MATH_SUCCESS) {
        printf(GREEN"\r\n STATUS -> SUCCESS"RESET_COLOR);
        printf("\r\n COMPLETE...");
        if (FAIL == fractCompare(0, 150, (fractional*)MADD_result_q31, (fractional*)MADD_er_q31)){
